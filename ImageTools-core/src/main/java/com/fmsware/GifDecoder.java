 ...
 * **
 * Notes:
 * Source found and copied from http://www.java2s.com/Code/Java/2D-Graphics-GUI/GiffileEncoder.htm
 *
 * Modifications:
 * Appropriate package chosen and package line added.
 * This list of notes.
 * logging
 * **
 * </pre>
...
public class GifDecoder {
    ...
    private static Logger logger = LoggerFactory.getLogger(GifDecoder.class);
            ...
            if (this.lastImage != null) {
                ...
                if (this.lastDispose == 2) {
                    ...
                    if (this.transparency) {
                        ...
                        c = new Color(this.lastBgColor); // use given background
                                                         // color
                    }
                    g.setColor(c);
        ...
        if (is != null) {
            ...
            if (!err()) {
                ...
                if (this.frameCount < 0) {
                    ...
                    this.logger.error("Error reading the gif.");
                }
            }
        } else {
            this.status = STATUS_OPEN_ERROR;
            this.logger.error("Error opening the gif.");
        }
        try {
            is.close();
        } catch (IOException e) {
            this.logger.warn("Could not close the input stream.", e);
        }
        return this.status;
    }
    ...
    protected int read() {
        ...
        try {
            ...
            this.status = STATUS_FORMAT_ERROR;
            this.logger.error("Error reading from the input stream", e);
        }
        return curByte;
    }
    ...
    protected int readBlock() {
        ...
        if (this.blockSize > 0) {
            try {
                ...
                this.logger.error("Error reading the block.", e);
            }

            if (n < this.blockSize) {
                this.status = STATUS_FORMAT_ERROR;
                this.logger.error("Unable to read the block.");
            }
        }
        return n;
    }
    ...
    protected void readHeader() {
        ...
        if (!id.startsWith("GIF")) {
            this.status = STATUS_FORMAT_ERROR;
            this.logger.error("Header id {} is invalid.", id);
            return;
        }
